...
func main() {
	...
	clientWithRequiredAuth, authErr := gocloudcix.NewClientWithAuth()
	if authErr != nil {
	...
	clientFromFile, fileErr := gocloudcix.NewClientFromFile("my_settings")
	if fileErr != nil {
	...
	clientWithSettings := gocloudcix.NewClientWithSettings(settings)
	...
	backups, err := client.Compute.Backups.List(ctx, gocloudcix.ComputeBackupListParams{})
	if err != nil {
	...
	if authErr == nil { // If clientWithRequiredAuth was created successfully
		instances, err := clientWithRequiredAuth.Compute.Instances.List(ctx, gocloudcix.ComputeInstanceListParams{})
		if err != nil {
	...
	if fileErr == nil {
		backups2, err := clientFromFile.Compute.Backups.List(ctx, gocloudcix.ComputeBackupListParams{})
		if err != nil {
